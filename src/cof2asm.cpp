...
const char *LookupOrdinal(char *NameBuffer,int size_nb,const char*DLLName,uint16 index)
{
	const char *result=0;
	struct ORD_TABLE{
		int index;
	...
	sprintf(NameBuffer+strlen(NameBuffer), "_Ordinal_%i", index);
	result = NameBuffer;

	struct ORD_TABLE table_comctl32[]={
		{17,"InitCommonControls"},
	};
	struct ORD_TABLE table_dplayx2[]={
		{1,"DirectPlayCreate"},
		{2,"DirectPlayEnumerate"},
	};
	struct ORD_TABLE *tbl=0;
	int tbl_size=0;
	if(strstri(DLLName,"COMCTL32")){
		tbl=table_comctl32;
		tbl_size=sizeof(table_comctl32)/sizeof(struct ORD_TABLE);
	}
	else if(strstri(DLLName,"DPLAYX")){
		tbl=table_dplayx2;
		tbl_size=sizeof(table_dplayx2)/sizeof(struct ORD_TABLE);
	}
	if(tbl!=0){
		int i;
...
void CCOF2ASM::MakeImportList() {
   ...
   while (ImportEntry->DLLNameRVA) {
      ...
      while (1) {
		 ...
		 {
			 SymbolName=LookupOrdinal(NameBuffer,sizeof(NameBuffer),DLLName,uint16(LookupEntry[0]));
         }
         else
		 {
...
void CCOF2ASM::MakeExportList() {
   ...
   if (OptionalHeader->h32.AddressOfEntryPoint) {
		const char	*entry_name="Entry_point";
		if(OptionalHeader->h32.Magic==0x010B)
			entry_name="WinMainCRTStartup";
      Disasm.AddSymbol(ASM_SEGMENT_IMGREL, OptionalHeader->h32.AddressOfEntryPoint, 0, 0x83, 4, 0, entry_name);
   }
