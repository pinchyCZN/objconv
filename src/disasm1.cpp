...
void CDisassembler::FindErrors() {
    ...
    if (IEnd > LabelEnd && IBegin < LabelEnd) {
        ...
        if (sym1 && (Symbols[sym1].Scope & 0x1C)) {
			...
			if (sym1){
				Symbols[sym1].Delta=LabelEnd-IBegin;
				//Symbols[sym1].Type = 0;       // Remove symbol type
				//Symbols[sym1].Section=-1;
			}
            //s.Warnings2 |= 1;
        }
    }
...
void CDisassembler::CheckLabel() {
    ...
    uint32 Sym1, Sym2, Next=0;

    // Find all symbol table entries at this address
    Sym1 = Symbols.FindByAddress(Section, IBegin, &Sym2,&Next);
	...
	if(Next && Symbols[Next].Delta){
		uint32 tmp=Symbols[Next].Offset-Symbols[Next].Delta;
		if(IBegin==tmp){
			if(!(Symbols[Next].Scope & 0x100)){
                WriteCodeLabel(Next);
			}
		}
	}
