...
void CDisassembler::WriteImmediateOperand(uint32 Type) {
    ...
    if ((Type & 0xFC) == 0x80 && !s.ImmediateRelocation) {
        ...
        if (ISymbol && (Symbols[ISymbol].Name || CodeMode == 1)) {
			if(Symbols[ISymbol].Delta){
				const char *n=Symbols.GetName(ISymbol);
				char tmp[64];
				_snprintf(tmp,sizeof(tmp),"%s+0x%X",n,Symbols[ISymbol].Delta);
				tmp[sizeof(tmp)-1]=0;
				OutFile.Put(tmp);
			}else{
...
void CDisassembler::WriteDataLabelYASM(const char * name, uint32 sym, int line) {
    ...
    if (line) {
        ...
        OutFile.NewLine();
		if(is_import){
			const char *LookupExport(const char*name);
			const char *tmp;
			OutFile.Tabulate(AsmTab1);
			OutFile.Put("dd ");
			tmp=LookupExport(name+4);
			OutFile.Put(tmp);
	        OutFile.NewLine();
		}
    }
...
void CDisassembler::WriteRelocationTarget(uint32 irel, uint32 Context, int64 Addend) {
    ...
    if (RelType & 0x100) {
		...
		if(Symbols[TargetSym].Delta){
			char tmp[64];
			_snprintf(tmp,sizeof(tmp),"+0x%X",Symbols[TargetSym].Delta);
			tmp[sizeof(tmp)-1]=0;
			OutFile.Put(tmp);
		}
